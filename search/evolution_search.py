...
from search import nsga2 as engine

from pymoo.core.problem import Problem
from pymoo.optimize import minimize
from nsga2 import NSGA2
from nsga2 import InitViT
...
parser.add_argument('--n_ops', type=int, default=13, help='number of operations considered')
parser.add_argument('--n_cells', type=int, default=1, help='number of cells to search')
...
parser.add_argument('--layers', type=int, default=6, help='equivalent with N = 3')
parser.add_argument('--epochs', type=int, default=25, help='# of epochs to train during architecture search')
parser.add_argument('--init_vit', action='store_true', default=False, help='# of epochs to train during architecture search')
parser.add_argument('--final_combiner', type=str, default='concat', help='concat or add')
parser.add_argument('--no_skip', action='store_true', default=False, help='removes skip connect between layers ')
...
class NAS(Problem):
    ...
    def __init__(self, search_space='micro', n_var=20, n_obj=1, n_constr=0, lb=None, ub=None,
                 init_channels=32, layers=6, epochs=25, save_dir=None, final_combiner = 'concat'):
        ...
        self.final_combiner = args.final_combiner
    def _evaluate(self, x, out, *args, **kwargs):
        ...
        for i in range(x.shape[0]):
            ...
            performance = train_search.main(genome=genome,
                                            ...
                                            final_combiner = self.final_combiner)
...
def main():
    ...
    if args.search_space == 'micro':  # NASNet search space
        n_var = int(4 * args.n_blocks)
                        # * args.n_cells)
        lb = np.zeros(n_var)
        ub = np.ones(n_var)
        h = 1
        if args.no_skip: #change lower bound from 0 to 1 for indices
    ...
    method = engine.NSGA2(pop_size=args.pop_size,
                          n_offsprings=args.n_offspring,
                          eliminate_duplicates=True,
                          init_vit = args.init_vit)
